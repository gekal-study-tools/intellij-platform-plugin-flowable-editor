...
プールとレーンは他の図形と同じように移動・大きさ変更・改名ができる。
ただし**プールを動かしても中の要素は付いてこない**。まとめて動かす仕組みも、
パレットからプールやレーンを新しく作る手段もまだ無い。
...
| アクティビティ | ユーザー / サービス / スクリプト / ビジネスルール / 受信 / 呼び出し / サブプロセス |
...
- パレットからプールやレーンを新しく作ることはできない（既にあるものの移動・
  大きさ変更・改名はできる）。プールを動かしても中の要素は付いてこない
- メッセージフローと関連（association）の作成は未対応
