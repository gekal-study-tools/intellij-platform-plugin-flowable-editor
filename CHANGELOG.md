...
- The palette gives every element its own icon, groups them by events, activities and gateways, and has a pointer tool (Escape also disarms).
  <br/>パレットは要素ごとに専用のアイコンを持ち、イベント・アクティビティ・ゲートウェイに分けて並ぶ。選択の道具を用意し、Esc でも解除できる。
