...
要素をレーンをまたいで動かすと、`flowNodeRef` も書き換わる。
図では移ったのに定義では元のまま、という食い違いを残さないため。
Flowable が見るのは定義のほうなので、見た目だけ合っている状態がいちばん厄介になる。
どのレーンにも入らない位置へ出せば、どのレーンにも属さなくなる。
...
**縦だけを元居たレーンの帯に戻す**。並べた結果が区画からはみ出すときは、
要素を外に出さずに区画のほうを広げる。
...
- 整列はレーンの帯の高さを変えない。帯より背の高い要素は帯からはみ出す
- 複数のプールにまたがる図で、要素を別のプールへ動かしてもプロセスの所属は変わらない
  （レーン所属は追従するが、プロセスをまたぐ移動は未対応）
