...
- macOS トラックパッドのピンチ操作による拡大縮小と、2 本指スクロールでの移動。Cmd/Ctrl + スクロールはトラックパッドではなめらかに追従する
...
- 開発用スクリプト `scripts/`（ビルド・テスト・整形・検証・サンドボックス起動・IDE への導入・描画確認・リリース）。すべて `--help` に対応し、`install.sh` は既定でビルドしてから導入、`--restart` で IDE の再起動まで行う
