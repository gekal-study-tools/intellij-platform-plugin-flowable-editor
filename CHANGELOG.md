...
- Pre-release channel publishing allocates a build number automatically (`0.1.0-eap.1`, `-eap.2`, ...), so the same version is never uploaded twice.
  <br/>事前公開チャンネルへの公開でビルド番号を自動採番する（`0.1.0-eap.1`, `-eap.2` ...）。同じ版を二度上げてしまうことがなくなる。
