...
- **手で決めた大きさは戻らない**。並べ直すのは位置であって大きさではない。
  ただし中身が入る大きさは要るので、計算結果より小さくはしない
- **両端が動かなかった線の折れ点は残る**。両端が動いた線は形が変わっているので引き直す
...
- 整列は要素を格子に合わせない。細かい位置合わせは手で行う
- 整列はレーンの並び順を変えない（帯の入れ替えは手で行う）
